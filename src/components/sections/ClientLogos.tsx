...
export default function ClientLogos() {
  ...
  // Duplicate logos for smooth infinite scrolling marquee
  const loopLogos = [...clientLogos, ...clientLogos, ...clientLogos, ...clientLogos];

  return (
    <section className="relative overflow-hidden py-6 md:py-8 border-b border-black" style={{ backgroundColor: "#F5F5F0" }}>
      ...
      <div className="md:ml-[max(1.5rem,min(5vw,4rem))] flex flex-col md:flex-row items-center gap-6 px-6 md:px-12">
        {/* Label on the left */}
        <div className="shrink-0 font-display font-semibold text-[13px] uppercase tracking-widest text-black/50 md:w-[160px] text-center md:text-left mb-2 md:mb-0">
          {language === "en" ? "Trusted Partners" : language === "gu" ? "વિશ્વસનીય ભાગીદારો" : "నమ్మకమైన భాగస్వాములు"}
        </div>
        
        {/* Scrolling Marquee */}
        <div className="flex-1 overflow-hidden w-full" style={{ maskImage: "linear-gradient(to right, transparent, black 15%, black 85%, transparent)" }}>
          <div className="flex gap-12 md:gap-16 items-center animate-marquee-left hover:play-state-paused" style={{ width: "max-content" }}>
            {loopLogos.map((logo, i) => (
              <div key={i} className="flex-shrink-0 flex items-center justify-center h-[40px] md:h-[48px] w-[120px] md:w-[140px]">
                <img
                  src={logo.src}
                  alt={logo.alt}
